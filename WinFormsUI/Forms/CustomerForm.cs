...
namespace WinFormsUI.Forms {
    ...
    #region MakeDraggable
                            ...
                            if (text.Substring(0, 1).ToLowerInvariant() == "C".ToLowerInvariant())
                                ...
                                _groupedCustomers = Customers
                                                    .Where(sp => sp.FullName.ToLowerInvariant().Contains(text)
                                                              ...
                                                              || sp.CompanyName.ToLowerInvariant().Contains(text))
                                                    .ToList();
        ...
        private void ViewEditCustomerButton_Click (object sender, EventArgs e) {
            try {
                Form ci =
                    new CustomerInformationForm(_groupedCustomers[CustomersDataGrid.SelectedCells[0].RowIndex]);
                    ...
                    if (row.Cells[0].Value.ToString().Contains("Sale"))
                        ...
                        Connection[0]
                            .DeleteDebtCollection(
                                                  _debtCollections.Single(dc => dc.DebtCollectionId
        ...
        private void PrintButton_Click (object sender, EventArgs e) {
            System.IO.Directory.CreateDirectory(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments)
                            + @"\Brotal\");
